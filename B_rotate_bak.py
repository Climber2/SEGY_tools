...
- center_file: 用于计算旋转中心的SEGY文件

描述：
此脚本用于处理SEGY格式数据的坐标旋转：
1. 使用内存映射方式读取SEGY文件
2. 批量读取和处理坐标数据
3. 使用numpy向量化操作进行坐标转换
4. 优化文件写入操作
...
def calculate_rotation_params(center_file, batch_size=100000):
    ...
    with segyio.open(center_file, "r", ignore_geometry=True) as center:
        if center.mmap():
            print(f"文件 {center_file} 已成功进行内存映射")
        else:
            print(f"文件 {center_file} 内存映射失败！")
        ...
        for batch_start in tqdm(range(0, num_traces, batch_size), desc="读取坐标"):
            ...
            sourceX_all[batch_start:batch_end] = center.attributes(segyio.TraceField.SourceX)[batch_start:batch_end]
            sourceY_all[batch_start:batch_end] = center.attributes(segyio.TraceField.SourceY)[batch_start:batch_end]
        
        # 对检波点坐标进行去重
        unique_coords = np.unique(np.column_stack((x_all, y_all)), axis=0)
        print(f"\n检波点数量: {len(x_all)} -> 去重后: {len(unique_coords)}")
        ...
        return group_centerX, group_centerY, rotation_matrix, x_all, y_all, sourceX_all, sourceY_all
...
def process_segy(filename, new_filename, center_file, batch_size=100000):
    # 计算旋转参数和获取所有坐标
    group_centerX, group_centerY, rotation_matrix, groupX, groupY, sourceX, sourceY = calculate_rotation_params(center_file, batch_size)
    ...
    plot_rotation_result(
        ...
        final_groupX, final_groupY, final_sourceX, final_sourceY,
        "../fig_0118/B_rotation_result.png"
    )
